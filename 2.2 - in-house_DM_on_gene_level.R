# aggregate cpg to gene-level using geometric mean 
# Library:
library(limma)
library(dbplyr)
library(lumi)
...
# Data:
#load myLoad, pd, myCombat from 1 - Raw data pre-processing.Rmd 
...
table_fit_genemean00 = topTable(fit_genemean00, num=Inf, coef=col_of_interest, adjust.method = "BH")
...
table_fitConfounders_genemean = topTable(fitConfounders_genemean, num=Inf, coef=col_of_interest, adjust.method = "BH")
...
# Make a volcano plot (label DMP with different colors)
pval2 = p.adjust(fitConfounders_genemean$p.value[,col_of_interest], "BH")
col_list2 = sapply(seq(1, dim(fitConfounders_genemean$coefficients)[1]), function(i) {
  if (pval2[i]<0.05) {return("red")}
  else {return("black")} })
cpg <- data.frame(logFC=fitConfounders_genemean$coefficients[,col_of_interest],pval=pval2)
...
table_fitAllConfounders_genemean = topTable(fitAllConfounders_genemean, num=Inf, coef=col_of_interest, adjust.method = "BH")
...
# Make a volcano plot (label DMP with different colors)
pval2 = p.adjust(fitAllConfounders_genemean$p.value[,col_of_interest], method = "fdr")
